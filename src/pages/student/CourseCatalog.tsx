...
export const CourseCatalogContent = ({ showHeader = true }: { showHeader?: boolean } = {}) => {
  const { user } = useUser();
  ...
  // Apply EN translations (when active language is English)
  const courseIds = (courses || []).map((c: any) => c.id);
  const { tCourseTitle, tCourseDescription } = useCourseI18n(courseIds);
  const localizedCourses = (courses || []).map((c: any) => ({
    ...c,
    title: tCourseTitle(c) || c.title,
    description: tCourseDescription(c) || c.description,
  }));
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("categories")
        .select("id, name, name_en, slug, display_order, is_active")
        .eq("is_active", true)
  ...
  // Localise category names when the active language is English.
  const localizeCatName = useInlineEnName();
  const localizedCategories = useMemo(
    () => (categories || []).map((c: any) => ({ ...c, name: localizeCatName(c) || c.name })),
    [categories, localizeCatName]
  );
  ...
  const filtered = localizedCourses.filter((c: any) => {
  ...
  localizedCourses.forEach((c: any) => {
      ...
      {showHeader && (
        ...
        {isLoading ? (
          ...
          <div className="min-w-0 space-y-8 sm:space-y-10">
            ...
            {localizedCourses.filter((c: any) => !c.category_id).length > 0 && (
              <CategoryCarousel
                category={{ id: "uncategorized", name: t("common.all") }}
                courses={localizedCourses.filter((c: any) => !c.category_id)}
