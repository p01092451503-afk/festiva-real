...
import { Settings, Bell, Shield, Building2, Plus, Pencil, Trash2, Palette, Users as UsersIcon, ToggleRight, CreditCard, EyeOff, Folder, Layers } from "lucide-react";
                  ...
                  {NAV_REGISTRY.map((group) => {
                    ...
                    return (
                      <div key={group.role} className="space-y-3">
                        <div className="flex items-center justify-between rounded-lg bg-primary/10 px-4 py-3 border border-primary/20">
                          ...
                          <span className="text-xs font-semibold text-primary bg-primary/15 rounded-full px-2.5 py-1">
                          ...
                          {group.categories.map((category) => {
                            ...
                            return (
                              <AccordionItem key={category.id} value={category.id} className="border-0">
                                <AccordionTrigger className="px-4 py-3.5 hover:no-underline bg-muted/50 data-[state=open]:bg-muted/70">
                                  <div className="flex items-center justify-between flex-1 gap-3 min-w-0">
                                    <div className="flex items-center gap-2.5 min-w-0">
                                      <Folder className="h-4 w-4 text-primary shrink-0" />
                                      <span className={`text-base font-semibold truncate ${allHidden ? "text-muted-foreground line-through" : "text-foreground"}`}>
                                        {category.label}
                                        {allHidden && (
                                    ...
                                    <span className="text-[11px] font-semibold text-muted-foreground whitespace-nowrap bg-muted rounded-full px-2.5 py-1">
