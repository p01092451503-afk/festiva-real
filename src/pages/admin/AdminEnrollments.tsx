...
import { Search, CheckCircle2, XCircle, Clock, Users, BookOpen, Filter, Hourglass } from "lucide-react";
...
import { ListBlockSkeleton } from "@/components/PageSkeletons";
  ...
  return (
    <DashboardLayout role="admin">
      <div className="space-y-6">
        ...
        {/* Stats — visualized */}
        {(() => {
          const totalE = pendingCount + approvedCount + rejectedCount;
          const pct = (n: number) => (totalE > 0 ? Math.round((n / totalE) * 100) : 0);
          return (
            <div className="grid grid-cols-3 gap-3">
              <RichStatCard
                label={t("enrollment.pendingCount")}
                value={pendingCount}
                icon={Hourglass}
                tone="amber"
                visual="ring"
                ringValue={pct(pendingCount)}
                sub={`${pct(pendingCount)}%`}
              />
              <RichStatCard
                label={t("enrollment.approvedCount")}
                value={approvedCount}
                icon={CheckCircle2}
                tone="emerald"
                visual="ring"
                ringValue={pct(approvedCount)}
                sub={`${pct(approvedCount)}%`}
              />
              <RichStatCard
                label={t("enrollment.rejectedCount")}
                value={rejectedCount}
                icon={XCircle}
                tone="rose"
                visual="ring"
                ringValue={pct(rejectedCount)}
                    ...
                    <tr key={enrollment.id} className="border-b border-border last:border-0 hover:bg-accent/30 transition-colors">
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground">{localizeByName(profile?.department) || ""} {profile?.position || ""}</p>
                        </div>
                      </td>
